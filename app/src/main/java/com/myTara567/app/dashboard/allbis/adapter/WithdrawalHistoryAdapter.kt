...
class WithdrawalHistoryAdapter(var hist_modelArrayList: ArrayList<wallet_withdrawal_hist_model>) :
    RecyclerView.Adapter<WithdrawalHistoryAdapter.wallet_transHist>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): wallet_transHist {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.withdrawel_request_history_item, parent, false)
    ...
    override fun onBindViewHolder(holder: wallet_transHist, position: Int) {
        ...
        holder.amount.text = "₹" + hist_model_list.request_amount
        holder.date.text = hist_model_list.insert_date
        holder.mTraId.text = hist_model_list.request_number
...
//        }
    ...
    class wallet_transHist(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var mTraId: TextView = itemView.findViewById(R.id.mTraId)
